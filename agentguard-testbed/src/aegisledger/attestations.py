...
Rfc3339Timestamp = Annotated[str, StringConstraints(min_length=20, max_length=40)]
...
class EnclaveEvidenceV1(StrictModel):
    ...
    @classmethod
    def preserve_datetime(cls, value: object) -> str:
    ...
    def expires_datetime(self) -> datetime:
        return datetime.fromisoformat(self.expires_at.replace("Z", "+00:00")).astimezone(UTC)

    def issued_datetime(self) -> datetime:
        return datetime.fromisoformat(self.issued_at.replace("Z", "+00:00")).astimezone(UTC)
...
class SettlementEvidenceV1(StrictModel):
    ...
    def normalize_time(cls, value: object) -> datetime:
        ...
        if not isinstance(value, datetime):
            raise TypeError("observed_at must be an ISO 8601 timestamp")
        if value.tzinfo is None or value.utcoffset() is None:
...
def verify_complete_attestation(
    ...
    *,
    ...
    issued_at = evidence.issued_datetime()
    expires_at = evidence.expires_datetime()
    if not verify_decision_token(decision, policy_public_key, now=issued_at):
    ...
    if evidence.transaction_hash != signed.transaction_hash:
        errors.append("enclave evidence transaction hash mismatch")
    if evidence.signing_hash != signed.eip1559_hash:
        errors.append("enclave evidence signing hash mismatch")
    ...
    if expires_at > decision.expires_at or expires_at > proposal.deadline:
    ...
    try:
        raw_transaction = bytes.fromhex(signed.raw_transaction[2:])
    except ValueError:
        errors.append("signed raw transaction is not hexadecimal")
    else:
        if "0x" + keccak(raw_transaction).hex() != signed.transaction_hash:
            errors.append("signed raw transaction hash mismatch")

    settlement = attestation.settlement
    if settlement is not None:
        if (
            settlement.transaction_hash != signed.transaction_hash
