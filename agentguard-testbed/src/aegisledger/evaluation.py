...
class ExperimentRunner:
    def run(self, spec: ExperimentSpecV1, output_directory: Path) -> ExperimentArtifacts:
        if output_directory.exists():
            return self._load_existing(spec, output_directory)
        ...
        try:
            _write_json(temporary / "manifest.json", spec.model_dump(mode="json"))
            _write_json(temporary / "summary.json", summary)
            (temporary / "raw_runs.jsonl").write_text(
    ...
    def _load_existing(
        ...
        if not all(path.is_file() for path in (manifest_path, summary_path, raw_path)):
            raise RuntimeError("existing experiment output is incomplete")
        manifest = ExperimentSpecV1.model_validate_json(manifest_path.read_text(encoding="utf-8"))
        if (
            manifest.experiment_id != spec.experiment_id
            or manifest.configuration_hash != spec.configuration_hash
            or manifest.commit_sha != spec.commit_sha
        ):
            raise RuntimeError("existing experiment output belongs to a different run")
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
        if summary.get("configuration_hash") != spec.configuration_hash:
            raise RuntimeError("existing experiment summary binding mismatch")
        raw_runs = tuple(
            json.loads(line) for line in raw_path.read_text(encoding="utf-8").splitlines()
        )
        if summary.get("raw_run_count") != len(raw_runs):
            raise RuntimeError("existing experiment raw-run count mismatch")
        return ExperimentArtifacts(spec, summary, raw_runs, output_directory)
