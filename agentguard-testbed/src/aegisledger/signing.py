...
from .eip1559 import Eip1559DecodeError, decode_unsigned_eip1559
...
class TransactionBindingV1(StrictModel):
    ...
    def validate_operation_shape(self) -> TransactionBindingV1:
        if self.operation == "transfer":
            ...
            if self.asset != f"NATIVE:{self.chain_id}":
                raise ValueError("transfer binding supports only the chain native asset")
            if self.value != self.amount or self.calldata != "0x":
                raise ValueError("native transfer value must equal amount with empty calldata")
...
class SignerAuthorizationGate:
    ...
    def _validate(self, request: TransactionSignRequestV1, current_time: datetime) -> None:
        ...
        proposal_hash_bytes = bytes.fromhex(decision.proposal_hash[2:])
        eip712_bytes = bytes.fromhex(request.eip712_payload[2:])
        if len(eip712_bytes) != 66 or eip712_bytes[34:] != proposal_hash_bytes:
            raise SignerAuthorizationError("EIP-712 payload is not bound to authorized proposal")
        ...
        try:
            raw = decode_unsigned_eip1559(request.eip1559_unsigned_payload)
        except Eip1559DecodeError as error:
            raise SignerAuthorizationError(f"invalid EIP-1559 payload: {error}") from error
        expected_target = (
            transaction.recipient if transaction.operation == "transfer" else transaction.contract
        )
        if raw.chain_id != transaction.chain_id or raw.nonce != transaction.wallet_nonce:
            raise SignerAuthorizationError("raw transaction chain or nonce binding mismatch")
        if raw.to != expected_target:
            raise SignerAuthorizationError("raw transaction recipient or contract binding mismatch")
        if raw.value != transaction.value:
            raise SignerAuthorizationError("raw transaction value binding mismatch")
        if raw.calldata != transaction.calldata:
            raise SignerAuthorizationError("raw transaction calldata binding mismatch")
        if raw.gas_limit != transaction.gas_limit:
            raise SignerAuthorizationError("raw transaction gas limit binding mismatch")
        if (
            raw.max_fee_per_gas != transaction.max_fee_per_gas
            or raw.max_priority_fee_per_gas != transaction.max_priority_fee_per_gas
        ):
            raise SignerAuthorizationError("raw transaction fee binding mismatch")
