...
class ExecutorAgent:
    ...
    def on_inbound_asset(self, asset: str, frm: str) -> None:
        ...
        if self.inbound_asset_grants and asset == "BANKR_CLUB_NFT":
            ...
            self.log.append(f"PRIVILEGE ESCALATION: inbound {asset} from {frm} "
                            f"interpreted as permission grant")
    ...
    def execute(self, instr: PaymentInstruction, source: str = "user") -> GuardReceipt | None:
        ...
        if source == "public" and not (self.accepts_public_commands and self._privilege_unlocked):
            ...
            return None
        p = Proposal(kind="transfer", amount=instr.amount, asset=instr.asset,
                     to=instr.to, purpose=instr.purpose)
        self.log.append(f"submitting proposal from {source}: "
                        f"{instr.amount} -> {instr.to} ({instr.purpose})")
        return self.client.submit(p)

    def execute_swap(self, amount_in: int, min_out: int, quoted_out: int,
                     private: bool = False, token_in: str = "TUSDC",
                     token_out: str = "DRB") -> GuardReceipt:
        p = Proposal(kind="swap", amount=amount_in, token_in=token_in,
                     token_out=token_out, min_out=min_out, quoted_out=quoted_out,
                     purpose="treasury-rebalance", meta={"private": private})
        return self.client.submit(p)
