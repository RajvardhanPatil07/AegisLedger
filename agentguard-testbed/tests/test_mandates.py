...
from agentwallet.payments.mandates import IntentMandate, CartMandate
...
def build(tb):
    ...
    intent = IntentMandate(user=user.address, agent=f"agent::{tb.seed}",
                           max_total=300_000_000, allowed_merchants=[merchant.address],
                           expires_at=tb.clock + 3600, nonce="i1").sign(user)
    cart = CartMandate(merchant=merchant.address, intent_hash=intent.hash(),
                       items=["dataset-q3"], total=TOTAL,
                       expires_at=tb.clock + 3600).sign(merchant)
    return user, merchant, intent, cart


def proposal(tb, amount=TOTAL, to=None):
    to = to or KeyPair.from_seed("vendor-merchant").address
    return Proposal(kind="transfer", amount=amount, to=to, purpose="buy",
                    meta={"agent": f"agent::{tb.seed}"})


def test_valid_chain_allows():
    tb = Testbed(mode=DefenseMode.GUARD_STRICT, seed="m1")
    user, merchant, intent, cart = build(tb)
    v = tb.guard.engine.evaluate(proposal(tb), mandate_chain=(
        intent, cart, {"user": user.pub, "merchant": merchant.pub}))
...
def test_amount_mismatch_with_cart_denied():
    ...
    user, merchant, intent, cart = build(tb)
    v = tb.guard.engine.evaluate(proposal(tb, amount=TOTAL + 1), mandate_chain=(
        intent, cart, {"user": user.pub, "merchant": merchant.pub}))
...
def test_expired_intent_denied():
    ...
    intent.sign(user)
    cart = CartMandate(merchant=merchant.address, intent_hash=intent.hash(),
                       items=["x"], total=TOTAL, expires_at=tb.clock + 3600).sign(merchant)
    v = tb.guard.engine.evaluate(proposal(tb), mandate_chain=(
        intent, cart, {"user": user.pub, "merchant": merchant.pub}))
